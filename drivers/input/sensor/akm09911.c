...
#define AKM_INPUT_DEVICE_UNIQ	"akm09911"
#define AKM_DRDY_TIMEOUT_MS		100
#define AKM_BASE_NUM			10

/* POWER SUPPLY VOLTAGE RANGE */
#define AKM09911_VDD_MIN_UV	2400000
#define AKM09911_VDD_MAX_UV	3600000
...
*/
	...
	(*input)->uniq = AKM_INPUT_DEVICE_UNIQ;
...
static int akm_compass_power_set(struct akm_compass_data *data, bool on)
{
	...
	if (!on) {
		...
		data->vdd = regulator_get(&data->i2c->dev, "akm,sensor_vdd");
		if (IS_ERR(data->vdd)) {
			rc = PTR_ERR(data->vdd);
		...
		data->vio = regulator_get(&data->i2c->dev, "akm,sensor_vcc_i2c");
		if (IS_ERR(data->vio)) {
			rc = PTR_ERR(data->vio);
			dev_err(&data->i2c->dev,
...
static int akm_compass_suspend(struct device *dev)
{
	...
	err = akm_compass_power_set(akm, 0);
	if(err < 0) {
		return err;
	}
...
static int akm_compass_resume(struct device *dev)
{
	...
	err = akm_compass_power_set(akm, 1);
	if(err < 0) {
		return err;
	}
				...
				struct akm_compass_data *pdata)
{
	...
	if (!gpio_is_valid(s_akm->gpio_rstn)) {
		...
		rc = gpio_request(s_akm->gpio_rstn, "akm_reset");
		if (rc < 0) {
			dev_err(dev, "gpio reset pin %d is invalid.\n",
			s_akm->gpio_rstn);
		}
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s_akm = devm_kzalloc(&client->dev, sizeof(struct akm_compass_data), GFP_KERNEL);
	if (!s_akm) {
		dev_err(&client->dev,
