...
// [LGE_UPDATE_S] [hyungtae.lee@lge.com] [2013-05-24] EEPROM ( of Y412B ) bring-up
// LGE_CHANGE. According to LOS, parameter name change memory_data to cal_data.mapdata. 2014.11.14. sujeong.kwon
static int msm_eeprom_check_CRC(struct msm_eeprom_ctrl_t *e_ctrl)
{
    ...
    CRC_code = (e_ctrl->cal_data.mapdata[0x037E] << 8) + e_ctrl->cal_data.mapdata[0x037F];
    CRC_sum = 0;

    for( counter = 0x000A; counter <= 0x037D; counter++ ) {
		CRC_sum  += e_ctrl->cal_data.mapdata[counter];
     }
    ...
    CRC_code = (e_ctrl->cal_data.mapdata[0x03C4] << 8) + e_ctrl->cal_data.mapdata[0x03C5];
    CRC_sum = 0;
...
#if defined (CONFIG_MACH_MSM8926_VFP_KR)
	pr_err("%s HI544 EEPROM VER: %d\n", __func__, e_ctrl->cal_data.mapdata[EEPROM_VER_ADDR]);
	if (e_ctrl->cal_data.mapdata[EEPROM_VER_ADDR] == 0) {
		//Data does NOT exist
		CDBG("%s HI544 EEPROM NOT Supported for 5100K!\n", __func__);
        rc_supported |= 0x00; //AWB bit Off
        rc_supported |= 0x00; //LSC bit Off
	}
	else if (e_ctrl->cal_data.mapdata[EEPROM_VER_ADDR] == 12) {
